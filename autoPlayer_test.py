...
class Ui_MainWindow(object):
    ...
    def go(self):
        ...
        while 1:
            try:
                just_cnt = driver.find_element_by_xpath('//*[@id="region-main"]/div/div[1]/div[2]/ul/li['+ str(max_class_index) +']/div/a/div[2]/div[2]')
                max_class_index += 1
            except:
                break
            ...
            if not CLASS_NAME[i] == '':
                try:
                    for j in range(1, max_class_index):
                        class_str = driver.find_element_by_xpath('//*[@id="region-main"]/div/div[1]/div[2]/ul/li[' + str(j) + ']/div/a/div[2]/div[2]/h3').text
                        if not class_str.find(CLASS_NAME[i]) == -1:
                            print('find class')
                            driver.find_element_by_xpath('//*[@id="region-main"]/div/div[1]/div[2]/ul/li['+ str(j) +']/div/a').click()
                            break
                except:
                ...
                while 1:                        
                    if section_now > STD_SECTION:
                        # reach STD_SECTION -> home back
                        try:
                        ...
                        try:
                            ...
                            driver.find_element_by_xpath('//*[@id="section-' + str(section_now) + '"]/div[3]/ul/li[' + str(video_index) + ']/div/div/div[2]/div/a').click()
                            time.sleep(3)
                            print('open video')
                            ...
                            try:
                                ...
                                play_btn = driver.find_element_by_xpath('//*[@id="vod_viewer"]')
                                play_btn.click()
                                print('play(not yet played video)')
                            except:
                                # already played
